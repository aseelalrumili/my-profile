      ...
      <form onSubmit={handleSubmit}>
        ...
        <div className="form-group">
          ...
          <label className="file-upload-area">
            <input type="file" multiple accept=".jpg,.jpeg,.png,.gif,.webp,.glb,.gltf,.obj,.fbx" style={{ display: 'none' }} onChange={(e) => setFiles(Array.from(e.target.files || []))} />
          ...
          <div style={{ display: 'flex', gap: '0.5rem', flexWrap: 'wrap', marginBottom: '1rem' }}>
            {project.media.map((m) => (
              <div key={m.id} style={{ position: 'relative', width: '70px', height: '70px', borderRadius: '8px', overflow: 'hidden', border: '1px solid var(--border)' }}>
                {m.mediaType === 'Image' ? <img src={getUploadUrl(m.url)} alt="" style={{ width: '100%', height: '100%', objectFit: 'cover' }} /> : <div style={{ width: '100%', height: '100%', display: 'flex', alignItems: 'center', justifyContent: 'center', background: 'var(--bg-secondary)', fontSize: '0.6rem', color: 'var(--text-muted)' }}>3D</div>}

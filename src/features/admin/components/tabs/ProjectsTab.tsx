...
function ProjectForm({ project, onSave, onClose, onDeleteMedia }: {
  ...
  return (
    <div className="admin-card-accent">
      <h4 className="admin-section-heading">{project ? t('admin.editProject') : t('admin.newProject')}</h4>
      <form onSubmit={handleSubmit}>
        <div className="admin-form-row">
          <div className="admin-form-group"><label>{t('admin.title')}</label><input type="text" value={title} onChange={(e) => setTitle(e.target.value)} required /></div>
          <div className="admin-form-group"><label>{t('admin.type')}</label><select value={type} onChange={(e) => setType(e.target.value as Project['type'])}><option value="Design">{t('admin.design')}</option><option value="Code">{t('admin.code')}</option><option value="Full-stack">{t('admin.fullstack')}</option></select></div>
        </div>
        <div className="admin-form-group"><label>{t('admin.description')}</label><textarea value={description} onChange={(e) => setDescription(e.target.value)} /></div>
        <div className="admin-form-row">
          <div className="admin-form-group"><label>{t('admin.category')}</label><input type="text" value={category} onChange={(e) => setCategory(e.target.value)} /></div>
          <div className="admin-form-group"><label>{t('admin.techStack')}</label><input type="text" value={techStack} onChange={(e) => setTechStack(e.target.value)} placeholder={t('admin.techStackPlaceholder')} /></div>
        </div>
        <div className="admin-form-group"><label>{t('admin.liveUrl')}</label><input type="url" value={liveUrl} onChange={(e) => setLiveUrl(e.target.value)} /></div>
        ...
        {project && project.media.length > 0 && (
          <div className="admin-media-grid">
            {project.media.map((m) => (
              <div key={m.id} className="admin-media-thumb">
                {m.mediaType === 'Image' ? <img src={m.url} alt="" /> : <div style={{ width: '100%', height: '100%', display: 'flex', alignItems: 'center', justifyContent: 'center', background: 'var(--bg-secondary)', fontSize: '0.6rem', color: 'var(--text-muted)' }}>3D</div>}
                <button type="button" onClick={() => onDeleteMedia(m.id)} className="admin-media-delete">x</button>
              </div>
            ))}
          </div>
        )}
        <div className="admin-btn-row">
          <button type="submit" className="btn btn-primary" disabled={isSaving}>{isSaving ? t('admin.saving') : project ? t('admin.update') : t('admin.create')}</button>
